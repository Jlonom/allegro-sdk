...
class GetQuantityModificationCommandTasksStatusesUsingGET extends \Jlonom\AllegroSDK\Runtime\Client\BaseEndpoint implements \Jlonom\AllegroSDK\Runtime\Client\Endpoint
{
    protected $commandId;
    /**
    ...
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if (401 === $status) {
            throw new \Jlonom\AllegroSDK\Exception\GetQuantityModificationCommandTasksStatusesUsingGETUnauthorizedException($response);
        }
        if (404 === $status) {
            throw new \Jlonom\AllegroSDK\Exception\GetQuantityModificationCommandTasksStatusesUsingGETNotFoundException($response);
        }
        if (422 === $status) {
            throw new \Jlonom\AllegroSDK\Exception\GetQuantityModificationCommandTasksStatusesUsingGETUnprocessableEntityException($response);
        }
    }
